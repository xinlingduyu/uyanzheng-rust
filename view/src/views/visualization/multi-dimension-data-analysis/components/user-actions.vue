<template>
  <a-card class="general-card" title="今日转评赞统计">
    <sa-chart height="122px" :options="chartOption" />
  </a-card>
</template>
...
const { chartOption } = useChartOption((isDark) => ({
  ...
  xAxis: {
    ...
    axisLabel: {
      show: true,
      formatter: (value, idx) => idx === 0 ? String(value) : `${Number(value) / 1000}k`
    },
  ...
  yAxis: {
    ...
    axisTick: { show: true, length: 2, lineStyle: { color: '#A9AEB8' }, alignWithLabel: true },

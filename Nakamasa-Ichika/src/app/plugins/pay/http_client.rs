...
/// 发送 POST 请求（JSON格式）
pub async fn post_json(url: &str, data: &serde_json::Value) -> Result<String, String> {
    let response = client()?
        .post(url)
...
/// 发送 POST 请求（空body，带Bearer Token认证）
pub async fn post_bearer(url: &str, token: &str) -> Result<String, String> {
    let response = client()?
        .post(url)
        .header("Content-Type", "application/json")
        .header("Authorization", format!("Bearer {}", token))

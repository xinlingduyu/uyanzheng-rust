    ...
    Router::with_path("/api/user/{appid}/{ver_key}/{ver_val}")
        ...
        .push(
            Router::with_path("/reEmail")
                .hoop(UserAuth::new())
                .post(profile::reEmail::re_email),
        )
        // 解绑手机 - 需要token
        .push(
            Router::with_path("/rePhone")
                .hoop(UserAuth::new())
                .post(profile::rePhone::re_phone),
        )
        // 获取验证码
        ...
        .push(
            Router::with_path("/wxBindSDK")
                .hoop(UserAuth::new())
                .post(oauth::wxBindSDK::wx_bind_sdk),
        )
        // QQ SDK绑定 - 需要token
        .push(
            Router::with_path("/qqBindSDK")
                .hoop(UserAuth::new())
                .post(oauth::qqBindSDK::qq_bind_sdk),
        )
        // 微信SDK登录
        ...
        .push(
            Router::with_path("/bindUdid")
                .hoop(UserAuth::new())
                .post(device::bindUdid::bind_udid),
        )
        // 获取设备列表 - 需要token
        .push(
            Router::with_path("/getUdid")
                .hoop(UserAuth::new().allow_udid())
                .post(device::getUdid::get_udid),
        )
        // 解绑设备 - 需要token
        .push(
            Router::with_path("/reUdid")
                .hoop(UserAuth::new().allow_udid())
                .post(device::reUdid::re_udid),
        )
        // 获取个人信息 - 需要token
        .push(
        ...
        .push(
            Router::with_path("/orderQuery")
                .hoop(UserAuth::new())
                .post(trade::orderQuery::order_query),
        )
        // 会员验证 - 需要token
        .push(
        ...
        .push(
            Router::with_path("/modifyName")
                .hoop(UserAuth::new())
                .post(profile::modifyName::modify_name),
        )
        // 修改头像 - 需要token
        .push(
            Router::with_path("/modifyPic")
                .hoop(UserAuth::new())
                .post(profile::modifyPic::modify_pic),
        )
        // 修改密码 - 需要token
        .push(
            Router::with_path("/modifyPwd")
                .hoop(UserAuth::new())
                .post(auth::modifyPwd::modify_pwd),
        )
        // 设置账号 - 需要token
        .push(
            Router::with_path("/setAcctno")
                .hoop(UserAuth::new())
                .post(profile::setAcctno::set_acctno),
        )
        // 绑定邮箱 - 需要token
        .push(
            Router::with_path("/setEmail")
                .hoop(UserAuth::new())
                .post(profile::setEmail::set_email),
        )
        // 设置扩展信息 - 需要token
        .push(
            Router::with_path("/setExtend")
                .hoop(UserAuth::new())
                .post(profile::setExtend::set_extend),
        )
        // 绑定手机号 - 需要token
        .push(
            Router::with_path("/setPhone")
                .hoop(UserAuth::new())
                .post(profile::setPhone::set_phone),
        )
        // 积分验证 - 需要token
        .push(
        ...
        .push(
            Router::with_path("/kamiTopup")
                .hoop(UserAuth::new())
                .post(trade::kamiTopup::kami_topup),
        )
        // 云函数 - 需要token
        .push(
            Router::with_path("/cloudFunction")
                .hoop(UserAuth::new())
                .post(misc::cloudFunction::cloud_function),
        )
        // 上传文件 - 需要token
        .push(
        ...
        .push(
            Router::with_path("/messageAdd")
                .hoop(UserAuth::new())
                .post(message::messageAdd::message_add),
        )
        .push(
            Router::with_path("/messageContent")
                .hoop(UserAuth::new())
                .post(message::messageContent::message_content),
        )
        .push(
            Router::with_path("/messageEnd")
                .hoop(UserAuth::new())
                .post(message::messageEnd::message_end),
        )
        .push(
            Router::with_path("/messageList")
                .hoop(UserAuth::new())
                .post(message::messageList::message_list),
        )
        .push(
            Router::with_path("/messageReply")
                .hoop(UserAuth::new())
                .post(message::messageReply::message_reply),
        )
        // 账户禁用 - 需要token
        .push(

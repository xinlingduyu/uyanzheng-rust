...
struct FenEvent {
    ...
    fen: i64,
    vip: i64,
    vip_free: &'static str,  // 'y' or 'n'
    state: &'static str,     // 'on' or 'off'
}
    ...
    if let Some(cached) = app_state.fen_event_cache.get(&fenid) {
        if cached.state == "on" {
            return Some(FenEvent {
                ...
                fen: cached.fen,
                vip: cached.vip,
                vip_free: Box::leak(cached.vip_free.clone().into_boxed_str()),
                state: Box::leak(cached.state.clone().into_boxed_str()),
            });
        }
        return None;
    }
    ...
    match result {
        Ok(Some((name, fen, vip, vip_free, state))) => {
            ...
            Some(FenEvent {
                ...
                fen,
                vip,
                vip_free: Box::leak(vip_free_str.into_boxed_str()),
                state: Box::leak(state_str.into_boxed_str()),
            })
        }
        _ => None,
    }

...
pub async fn get_list(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    let mut sql_query = sqlx::query_as::<_, (i64, i64, i64, Option<String>, Option<i64>, Option<i64>, String, String)>(&query);
    for param in params {
    ...
    match result {
        Ok(rows) => {
            let list: Vec<AgentListItem> = rows.into_iter().map(|row| {
                AgentListItem {
                    id: row.0,
                    aggid: row.1,
                    uid: row.2,
...
pub async fn add(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    let insert_result = sqlx::query(
        "INSERT INTO u_agent (aggid, uid, note, appid) VALUES (?, ?, ?, ?)"
    )
    .bind(add_req.gid)
...
pub async fn edit(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    let result = sqlx::query(
        "UPDATE u_agent SET aggid = ?, note = ?, money = ? WHERE id = ?"
    )
    .bind(edit_req.aggid)
    .bind(edit_req.note)
    .bind(edit_req.money)

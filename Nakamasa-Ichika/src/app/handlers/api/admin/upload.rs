...
pub async fn img(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    tracing::info!("上传基础目录: {}", upload_base_dir);
    ...
    tracing::info!("开始解析表单数据...");
    let form_data = match req.form_data().await {
        Ok(data) => {
            tracing::info!("表单数据解析成功，文件数量: {}", data.files.len());
            data
        },
        Err(e) => {
    ...
    for (name, files) in form_data.files.iter_all() {
        tracing::info!("文件字段: {}, 数量: {}", name, files.len());
        for (i, file) in files.iter().enumerate() {
            tracing::info!("  文件 {}: name={:?}, size={}, content_type={:?}", 
    ...
    tracing::info!("读取文件内容，路径: {:?}", file.path());
    let file_data = match fs::read(file.path()) {
        Ok(data) => {
            tracing::info!("文件读取成功，大小: {} bytes", data.len());
            data
        },
        Err(e) => {
    ...
    // 保存文件
    tracing::info!("保存文件到: {:?}", canonical_file_path);

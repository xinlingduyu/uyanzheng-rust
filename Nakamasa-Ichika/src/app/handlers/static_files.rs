...
/// 上传目录的基础路径（相对于项目根目录）
const UPLOAD_BASE_DIR: &str = "./data/upload";
...
#[handler]
pub async fn admin_handler(req: &mut Request, depot: &mut Depot, res: &mut Response, ctrl: &mut FlowCtrl) {
    // 获取请求路径
...
#[handler]
async fn root_handler(req: &mut Request, depot: &mut Depot, res: &mut Response, ctrl: &mut FlowCtrl) {
    let path = req.uri().path();
    // 跳过 /admin 和 /static 路径
    if path.starts_with("/admin") || path.starts_with("/static") {
        ctrl.call_next(req, depot, res).await;
    } else {
        StaticDir::new(["Nakamasa-Ichika/static"]).fallback("index.html").handle(req, depot, res, ctrl).await;
    }
}

/// 静态文件路由
pub fn static_files_route() -> Router {
    Router::with_path("/static/{**rest}")
        .get(StaticDir::new(["Nakamasa-Ichika/static"]))
}
...
/// 4. 只允许访问特定扩展名的文件
/// 5. 隐藏文件（以`.`开头）不允许访问
fn validate_upload_path(request_path: &str) -> Result<PathBuf, String> {
    // 1. 检查空字节注入
...
/// - 路径规范化验证
#[handler]
pub async fn upload_file_handler(req: &mut Request, _depot: &mut Depot, res: &mut Response) {
    let request_path = req.uri().path();
    
    tracing::debug!("上传文件访问请求: {}", request_path);
    
    // 安全验证路径
    let file_path = match validate_upload_path(request_path) {
        Ok(path) => path,
        Err(e) => {
    ...
    // 设置响应头
    res.headers_mut().insert("Content-Type", mime_type.parse().unwrap());
    res.headers_mut().insert("Cache-Control", "public, max-age=86400".parse().unwrap()); // 缓存1天
    res.headers_mut().insert("ETag", etag.parse().unwrap());
...
pub fn upload_files_route() -> Router {
    Router::with_path("/upload/{**rest}")
        .get(upload_file_handler)
}
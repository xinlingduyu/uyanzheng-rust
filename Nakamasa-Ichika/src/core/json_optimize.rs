...
macro_rules! json_success {
    ($data:expr) => {{
        $crate::core::json_optimize::success_response($data)
    }};
}
...
macro_rules! json_error {
    ($message:expr, $code:expr) => {{
        $crate::core::json_optimize::error_response($message, $code)
    }};
}
...
macro_rules! json_error_static {
    ($message:expr, $code:expr) => {{
        $crate::core::json_optimize::error_response_static($message, $code)
    }};
}
...
impl StreamingJson {
    /// 流式处理大 JSON 文件
    pub async fn process_large_json<F, R>(path: &str, mut processor: F) -> Result<Vec<R>, Box<dyn std::error::Error>>
    where
...
mod tests {
    ...
    fn test_fast_json() {
        ...
        assert_eq!(FastJson::extract_string(&value, "name"), Some(Cow::Borrowed("test")));
